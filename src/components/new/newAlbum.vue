<template>
  <div class="newSong">
    <div class="content">
      <div class="container">
        <ul v-for="(newAlbum,index) in reNewAlbums" :key="index">
          <li v-for="(item,index) in newAlbum" :key="index" @click="goPlay(item)">
            <div>
              <a>
                <img :src="item.picUrl"/>
              </a>
...
<script>
  ...
  export default {
    computed: {
      ...mapState({
        newAlbums: (state) => state.newAlbum,
      }),
      reNewAlbums(){
        let a = _.drop(this.newAlbums,6)
        let b = _.chunk(a,3 )
        // console.log(b)
        return b
      },
      saveSongDetail(){
      let d = this.newAlbums.map((song)=>{
        let reSong ={}
        reSong.id = song.id
    ...
    mounted() {
      this.$store.dispatch("getNewAlbumAction");
    },
    watch: {
      reNewAlbums() {
        this.$nextTick(() => {
